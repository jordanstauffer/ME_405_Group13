import pyromat as pm
import numpy as np
import matplotlib.pyplot as plt
import numpy_financial as npfin

Tmax = 625 #celcius
Tsat = 295.01
Pmax = 15*1000 #kPa
Pcond = 8.209798435 #kPa
Preheat = 8*1000 #kPa

power = 35 #megaWatts
plantprice = 50000000 #dollars
#over 25 years

fuelcost = [3.66,3.54,3.34,2.96,2.86,2.72,2.55,2.91,3.16,2.35,3.93,4.07] ###natural gas cost $/1000ft^3
sunhours = [5,6,7,9,14,16,15,13,10,8,7,6] # at 800W/m^2
saleprice = [47,47.3,48.6,47.7,46.8,48.3,53.5,52.8,58.2,57.2,56.8,53.8] #$/MWh

days = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
hours = [ds*24 for ds in days]
hourssun = [[ds*sunhours[i] for i in sunhours] for ds in days] #hours of operation per month

inflationsale = 0.03
inflationcost = 0.05

nt = [0.8,0.85,0.9,0.95]
highpress = [1500000,2000000,2500000,3000000]
midpress = highpress
lowpress = midpress
#combine all costs for no reheat, combine high and mid for 1st reheat and low for 2nd reheat

Tcond = [45,35,30,25]
condensorcost = [12000000,25000000,30000000,40000000]

npump = [0.75,0.8,0.85,0.9]
pumpcost = [25000,30000,35000,40000]

#####84 turbine options#######
#4 options with no reheat: use combined cost of high, mid, and low
#16 options with 1 reheat: use 0.1 to 4 MPa for Preheat. use high and mid cost for 1st stage (boiler, turbine), and low cost for 2nd stage (boiler, turbine)
                           # 
#64 options with 2 reheat: 1st stage is 1-5 MPa, 2nd stage is 0.1-0.5MPa. use high and mid cost for 1st stage (boiler,trurbine), and low cost for 2nd stage (boiler,turbine)
                           #
#open feedwater heaters, if included, must be same pressure as reheat. can include condensor temp and pump efficiency

steam = pm.get('mp.H2O')
units = {'temperature':'C','pressure':'kPa'} #change units
for k,v in units.items():
    pm.config['unit_'+k] = v

#boiler
def isobaric_heat_addition(T,P):
    Temp = T
    Pressure = P
    h = steam.h(T=Temp,p=Pressure)
    s = steam.s(T=Temp,p=Pressure)
    v = steam.v(T=Temp,p=Pressure)
    x=None
    return Temp,Pressure,h,s,v,x

#turbine
def adiabatic_expansion(nt,h,P,s):
    Pressure = P
    sf,sg = steam.ss(p=Pressure)
    hf,hg = steam.hs(p=Pressure)
    snew=s   
    for i in h:
        for j in snew:
            if np.any(j>sg):
                hs = steam.h(s=j,p=P)
                hnew = i-nt*(i-hs)
                Temp = steam.T(h=hnew,p=Pressure)
            else:
                x = (j-sf)/(sg-sf)
                hs = hf+x*hg
                hnew = i-nt*(i-hs)
                Temp = steam.Ts(p=Pressure)
                
            if np.any(hnew>hg):
                v = steam.v(T=Temp,p=Pressure)
                snew = steam.s(T=Temp,p=Pressure)
                x = None

            else:
                x = (hnew-hf)/(hg-hf)
                v = steam.v(p=Pressure,x=x)
                snew = steam.s(p=Pressure,x=x)
            if x>0.95:
                print('quality cannot be below 95%')
                break
    return Temp, Pressure, h,s,v,x

#condensor
def get_Saturated_state(T,P):
    Temp=T
    Pressure=P
    hf,hg = steam.hs(Temp)
    sf,sg = steam.ss(Temp)
    vf,vg = steam.vs(Temp)
    s=sf
    h=hf
    x=0
    v=vf
    return Temp, Pressure,h,s,v,x

#pump
def adiabatic_compression(Pf,Pmax,npump,v,h):
    Pressure=Pmax
    hs = h+v*(Pressure-Pf) 
    hnew = h+(hs-h)/npump 
    Temp = steam.T(h=hnew,p=Pressure) #may be wrong
    vnew = v
    s=steam.s(T=Temp,p=Pressure)
    x=None
    return Temp,Pressure,hnew,s,vnew,x

def mixing(T,P):
    Pressure = P
    Temp = T
    hf,hg= steam.hs(T=Temp)
    sf,sg= steam.ss(T=Temp)
    vf,vg = steam.vs(T=Temp)
    h = hf
    s=sf
    v=vf
    x=0
    return Temp,Pressure,h,s,v,x

#no reheat
T1,P1,h1,s1,v1,x1 = get_Saturated_state(Tcond,Pcond) #condensor
T2,P2,h2,s2,v2,x2 = adiabatic_compression(P1,Preheat,npump,v1,s1,h1) #pump 1
T3,P3,h3,s3,v3,x3 = isobaric_heat_addition(Tmax,Pmax) #boiler 1
T4,P4,h4,s4,v4,x4 = adiabatic_expansion(nt,h3,Pcond,s3) #turbine 1

#1 reheat stage - nt will change    
T1,P1,h1,s1,v1,x1 = get_Saturated_state(Tcond,Pcond) #condensor
T2,P2,h2,s2,v2,x2 = adiabatic_compression(P1,Preheat,npump,v1,s1,h1) #pump 1
T3,P3,h3,s3,v3,x3 = isobaric_heat_addition(Tmax,Pmax) #boiler 1            STAGE 1
T4,P4,h4,s4,v4,x4 = adiabatic_expansion(nt,h4,Preheat,s4) #turbine 1
T5,P5,h5,s5,v5,x5 = isobaric_heat_addition(Tmax,Preheat)#boiler 2
T6,P6,h6,s6,v6,x6 = adiabatic_expansion(nt,h5,Pcond,s5)#turbine 2

#2 reheat stages - nt will change. pressures for stage 1 and 2 differ 
T1,P1,h1,s1,v1,x1 = get_Saturated_state(Tcond,Pcond) #condensor
T2,P2,h2,s2,v2,x2 = adiabatic_compression(P1,Preheat,npump,v1,s1,h1) #pump 1
T3,P3,h3,s3,v3,x3 = isobaric_heat_addition(Tmax,Pmax) #boiler 1            STAGE 1
T4,P4,h4,s4,v4,x4 = adiabatic_expansion(nt,h4,Preheat,s4) #turbine 1
T5,P5,h5,s5,v5,x5 = isobaric_heat_addition(Tmax,Preheat)#boiler 2
T6,P6,h6,s6,v6,x6 = adiabatic_expansion(nt,h6,Preheat,s6)#turbine 2
T7,P7,h7,s7,v7,x7 = isobaric_heat_addition(Tmax,Preheat)#boiler 3          STAGE 2
T8,P8,h8,s8,v8,x8 = adiabatic_expansion(nt,h7,Preheat,s7)#turbine 3
T9,P9,h9,s9,v9,x9 = isobaric_heat_addition(Tmax,Preheat)#boiler 4
T10,P10,h10,s10,v10,x10 = adiabatic_expansion(nt,h9,Pcond,s9)#turbine 4

#need to define pressures, enthalpies, and entropies separately in lists for 0,1,or 2 reheat stages

#where h is a list that changes depending on case
#####if using feedwater######
# massfraction = []
# if len(h) >= 7:
#     massfraction = 1-((h[2]-h[1])/(h[5]-h[1]))  #ask about in class
#     massfraction = np.array([float(massfraction),float(massfraction),float(1),float(1),float(1),float(massfraction),float(massfraction),float(massfraction)])

def special_calculations(power,P,s,h):
    specificwork = []
    specificheat = []

    for i in range(len(h)-1):
        if abs(P[i+1]-P[i])>0.001:
            specificwork.append(float(h[i+1]-h[i]))
        if abs(P[i+1]-P[i])<=0.001 and s[i+1]>s[i]:
            specificheat.append(float(h[i+1]-h[i]))
        
    network = np.sum(specificwork)
    netheat = np.sum(specificheat)
    flow = power/(specificwork[1]*100)
    totalwork = [flow*specificwork[i] for i in range(len(specificwork))]
    totalheat = [flow*specificheat[i] for i in range(len(specificheat))]

    n_total = abs(network/netheat)
    return n_total,totalheat
n_total,totalheat= special_calculations(power,P,s,h)

#total heat is used for cost calculations


#######order of operations#########
# 1. determine best turbine 
# 2. determine best 



# T1,P1,h1,s1,v1,x1 = get_Saturated_state(Tcond,Pcond) #condensor
# T2,P2,h2,s2,v2,x2 = adiabatic_compression(P1,Preheat,npump,v1,s1,h1) #pump 1
# T3,P3,h3,s3,v3,x3 = isobaric_heat_addition(Tmax,Pmax) #boiler 1
# T4,P4,h4,s4,v4,x4 = adiabatic_expansion(nt,h3,Pcond,s3) #turbine 1